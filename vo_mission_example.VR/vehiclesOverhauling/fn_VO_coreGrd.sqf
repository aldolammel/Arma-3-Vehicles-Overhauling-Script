...
if ( !isServer || !VO_isOn || !VO_groundDoctrine ) exitWith {};

// GROUND SERVICES CORE / BE CAREFUL BELOW:
[] spawn {

	private ["_fullAssets","_repAssets","_refAssets","_reaAssets","_fullAndRepAssets","_fullAndRefAssets","_fullAndReaAssets","_allAssets","_isServProgrs","_players","_eachPlayer","_playerVehList","_currentPlayerVehList","_eachVeh"];
	...
	if VO_debug_isOn then { VO_grdStationsAmount = count _allAssets }; 
	...
	while { VO_isStationsOkay && VO_isServicesOkay } do {
		...
		// debug:
		if VO_debug_isOn then { call THY_fnc_VO_debugMonitor; VO_grdCyclesDone = (VO_grdCyclesDone + 1) };
