...
// EDITOR'S OPTIONS:
	...
	// AIR DOCTRINE SERVICES
	VO_airDoctrine = true;           // true = the mission needs air stations / false = doesn't need.
	...
	// NAUTIC DOCTRINE SERVICES
	VO_nauticDoctrine = true;         // true = the mission needs nautic stations / false = doesn't need.
	...
	// BE CAREFUL, HIGHLY RECOMMENDED DO NOT CHANGE ANY BELOW:
		...
		if (VO_groundDoctrine OR VO_airDoctrine OR VO_nauticDoctrine) then {
			...
			if ( VO_groundDoctrine ) then {
				...
				if ( VO_grdServRepair OR VO_grdServRefuel OR VO_grdServRearm ) then {VO_isServicesOkay = true} else {systemChat "VO > fn_VO_parameters.sqf > No GROUND services set as TRUE!"}; 
				if ( VO_grdServRepair && count (VO_grdRepairAssets + VO_grdFullAssets) == 0 ) then {systemChat "VO > fn_VO_parameters.sqf > No GROUND REPAIR assets configured!"};
				if ( VO_grdServRefuel && count (VO_grdRefuelAssets + VO_grdFullAssets) == 0 ) then {systemChat "VO > fn_VO_parameters.sqf > No GROUND REFUEL assets configured!"};
				if ( VO_grdServRearm && count (VO_grdRearmAssets + VO_grdFullAssets) == 0 ) then {systemChat "VO > fn_VO_parameters.sqf > No GROUND REARM assets configured!"}};
			if ( VO_airDoctrine ) then {
				...
				if ( VO_airServRepair OR VO_airServRefuel OR VO_airServRearm ) then {VO_isServicesOkay = true} else {systemChat "VO > fn_VO_parameters.sqf > No AIR services set as TRUE!"};
				if ( VO_airServRepair && count (VO_airRepairAssets + VO_airFullAssets) == 0 ) then {systemChat "VO > fn_VO_parameters.sqf > No AIR REPAIR assets configured!"};
				if ( VO_airServRefuel && count (VO_airRefuelAssets + VO_airFullAssets) == 0 ) then {systemChat "VO > fn_VO_parameters.sqf > No AIR REFUEL assets configured!"};
				if ( VO_airServRearm && count (VO_airRearmAssets + VO_airFullAssets) == 0 ) then {systemChat "VO > fn_VO_parameters.sqf > No AIR REARM assets configured!"}};
			if ( VO_nauticDoctrine ) then {
				...
				if ( VO_nauServRepair OR VO_nauServRefuel OR VO_nauServRearm ) then {VO_isServicesOkay = true} else {systemChat"VO > fn_VO_parameters.sqf > No NAUTIC services set as TRUE!"};
				if ( VO_nauServRepair && count (VO_nauRepairAssets + VO_nauFullAssets) == 0 ) then {systemChat "VO > fn_VO_parameters.sqf > No AIR REPAIR assets configured!"};
				if ( VO_nauServRefuel && count (VO_nauRefuelAssets + VO_nauFullAssets) == 0 ) then {systemChat "VO > fn_VO_parameters.sqf > No AIR REFUEL assets configured!"};
				if ( VO_nauServRearm && count (VO_nauRearmAssets + VO_nauFullAssets) == 0 ) then {systemChat "VO > fn_VO_parameters.sqf > No AIR REARM assets configured!"}};
